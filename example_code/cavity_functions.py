...
def make_cavity_v4(fullfilepath,waveguide_width,taper_length):
    """_summary_

    Args:
        gds_file_name (string): the file name of the cavity gds generated by waveguide solver
        waveguide_width (float with unit um): the end width of the tapping waveguide region
        taper_length (float with unit um): the length of the taper fiber interface
    Returns:
        _type_: _description_
    """
    ...
    # all in um 
    trenchW = 6
    wvg_length = 2 # reduced from 5 to 2
    teth_length = 10
    teth_width = 1.3*cavity_width
    ...
    # make the taper waveguide region
    tap_wvg = pg.taper(taper_length,width1=cavity_width,width2=waveguide_width)
    tap_wvg.add_port('tap_wvg_left',(0,0), orientation= 180)
    ...
    tap_ref = E.add_ref(tap_wvg)
    tap_ref.connect('tap_wvg_left', destination=teth_ref1.ports['tether_right'])
    full_dev_width = 5 + abs(xL)+xR + teth_length+wvg_length + taper_length + 10
    trench_rect = pg.rectangle((full_dev_width,trenchW)).move((xL,-trenchW/2))
...
def gen_gds_list(gdsdir,gdsfiles):
    ...
    for i in range(len(gdsfiles)):
        for j in range(len(w_list)):
            ...
            dev = make_cavity_v4(fullfilepath,w_list[j],taper_length=30)
...
def gen_gds_list_v2(gdsdir,gdsfiles):
    """generating an array of devices with varying taper interface length

    Args:
        gdsdir (_type_): _description_
        gdsfiles (_type_): _description_
    ...
    # list of minimum waveguide width we are sweeping through 
    w_list = [0.06,0.120,0.180] # in um
    taper_length_list = [10,20,30] # in um 
    for i in range(len(gdsfiles)):
        for j in range(len(taper_length_list)):
            fullfilepath = gdsdir+gdsfiles[i]
            dev = make_cavity_v4(fullfilepath,w_list[1],taper_length_list[j])
            nums = re.findall(r'[\d]+',gdsfiles[i])
            v_num = nums[0]
            a_num = nums[1]
            w_num = str(j+1)
